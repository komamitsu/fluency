...
class FlusherTest {
  ...
  void queueSizeShouldNotExceedLimit() throws Exception {
    ...
    BlockingQueue<Boolean> eventQueue =
        (BlockingQueue<Boolean>) extractValue(flusher, "eventQueue");
    ...
    // Wait for sufficiently long (amount of time > queue poll wait) and the queue should be polled
    // completely to become empty
    Thread.sleep(1000);
