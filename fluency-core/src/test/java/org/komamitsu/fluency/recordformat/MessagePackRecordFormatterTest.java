...
class MessagePackRecordFormatterTest {
  ...
  private static final Map<String, Object> RECORD_2 =
      ImmutableMap.of("job", "knight", "name", "third", "age", 99);
  ...
  private void assertRecord0(byte[] formatted, long expectedTime) throws IOException {
  ...
  private void assertRecord1(byte[] formatted, long expectedTime) throws IOException {
    try (MessageUnpacker unpacker = MessagePack.newDefaultUnpacker(formatted)) {
      Map<Value, Value> map = unpacker.unpackValue().asMapValue().map();
      assertEquals(4, map.size());
      assertEquals(expectedTime, map.get(KEY_TIME).asNumberValue().toLong());
  ...
  private void assertRecord2(byte[] formatted, long expectedTime) throws IOException {
    try (MessageUnpacker unpacker = MessagePack.newDefaultUnpacker(formatted)) {
      Map<Value, Value> map = unpacker.unpackValue().asMapValue().map();
      assertEquals(4, map.size());
      assertEquals(expectedTime, map.get(KEY_TIME).asNumberValue().toLong());
  ...
  @Test
  void formatFromMessagePackBytes() throws IOException {
    long now = System.currentTimeMillis() / 1000;
    ...
    }
    {
      ...
      assertRecord1(
          recordFormatter.formatFromMessagePack(TAG, now, bytes, 0, bytes.length), FUTURE_EPOCH);
    }
    {
  ...
  @Test
  void formatFromMessagePackByteBuffer() throws IOException {
    long now = System.currentTimeMillis() / 1000;
    assertRecord0(
