...
public class FileBackup {
  ...
  public FileBackup(File backupDir, Buffer userBuffer, String prefix) {
    ...
    if (!backupDir.isDirectory() || !backupDir.canRead() || !backupDir.canWrite()) {
      throw new IllegalArgumentException(
          "backupDir[" + backupDir + "] needs to be a readable & writable directory");
    }
  ...
  public List<SavedBuffer> getSavedFiles() {
    ...
    for (File f : files) {
      ...
      if (matcher.find()) {
        ...
        LOG.trace(
            "Found a file in backup dir, but the file path doesn't match the pattern. file={}",
            f.getAbsolutePath());
      }
    }
    return savedBuffers;
  ...
  public void saveBuffer(List<String> params, ByteBuffer buffer) {
    ...
    String filename =
        this.userBuffer.bufferFormatType()
            + prefix()
            + PARAM_DELIM_IN_FILENAME
            + sb.toString()
            + EXT_FILENAME;
    ...
    try {
      ...
      LOG.error(
          "Failed to save buffer to file: params={}, path={}, buffer={}",
          copiedParams,
  ...
  public static class SavedBuffer implements Closeable {
    ...
    @Override
    public String toString() {
      return "SavedBuffer{" + "params=" + params + ", savedFile=" + savedFile + '}';
    }
