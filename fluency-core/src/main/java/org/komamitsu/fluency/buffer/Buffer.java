...
public class Buffer implements Closeable {
  ...
  public Buffer(final Config config, RecordFormatter recordFormatter) {
    ...
    if (config.getFileBackupDir() != null) {
      fileBackup =
          new FileBackup(new File(config.getFileBackupDir()), this, config.getFileBackupPrefix());
    } else {
      fileBackup = null;
    }
  ...
  private RetentionBuffer prepareBuffer(String tag, int writeSize) throws BufferFullException {
    ...
    if (retentionBuffer == null) {
      ...
      newBufferChunkRetentionSize =
          (int) (retentionBuffer.getByteBuffer().capacity() * config.getChunkExpandRatio());
    }
    ...
    if (acquiredBuffer == null) {
      throw new BufferFullException(
          "Buffer is full. config=" + config + ", bufferPool=" + bufferPool);
    }
  ...
  private void appendMessagePackMapValueInternal(
      String tag, Object timestamp, byte[] mapValue, int offset, int len) throws IOException {
    loadDataToRetentionBuffers(
        tag,
        ByteBuffer.wrap(
            recordFormatter.formatFromMessagePack(tag, timestamp, mapValue, offset, len)));
  }

  private void appendMessagePackMapValueInternal(String tag, Object timestamp, ByteBuffer mapValue)
      throws IOException {
    loadDataToRetentionBuffers(
        tag, ByteBuffer.wrap(recordFormatter.formatFromMessagePack(tag, timestamp, mapValue)));
  }
  ...
  public void appendMessagePackMapValue(
      String tag, long timestamp, byte[] mapValue, int offset, int len) throws IOException {
    appendMessagePackMapValueInternal(tag, timestamp, mapValue, offset, len);
  }

  public void appendMessagePackMapValue(
      String tag, EventTime timestamp, byte[] mapValue, int offset, int len) throws IOException {
    appendMessagePackMapValueInternal(tag, timestamp, mapValue, offset, len);
  }
  ...
  protected void flushInternal(Ingester ingester, boolean force) throws IOException {
    ...
    while (!Thread.currentThread().isInterrupted()
      ...
      try {
        LOG.trace(
            "flushInternal(): bufferUsage={}, flushableBuffer={}",
            getBufferUsage(),
            flushableBuffer);
        ...
        LOG.warn(
            "Failed to send data. The data is going to be saved into the buffer again: data={}",
            flushableBuffer);
        keepBuffer = true;
        throw e;
      } finally {
        if (keepBuffer) {
          try {
            ...
            LOG.warn(
                "Failed to save the data into the buffer. Trying to save it in extra buffer: chunk={}",
                flushableBuffer);
  ...
  private static class TaggableBuffer {
    ...
    @Override
    public String toString() {
      return "TaggableBuffer{" + "tag='" + tag + '\'' + ", byteBuffer=" + byteBuffer + '}';
    }
  }
