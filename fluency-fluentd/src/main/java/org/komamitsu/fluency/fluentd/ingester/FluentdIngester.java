  ...
  public void ingest(String tag, ByteBuffer dataBuffer) throws IOException {
    ...
    if (config.isAckResponseMode()) {
      ...
      ByteBuffer optionBuffer =
          ByteBuffer.wrap(objectMapper.writeValueAsBytes(new RequestOption(dataLength, token)));
      ...
      ByteBuffer optionBuffer =
          ByteBuffer.wrap(objectMapper.writeValueAsBytes(new RequestOption(dataLength, null)));
      List<ByteBuffer> buffers = Arrays.asList(headerBuffer, dataBuffer, optionBuffer);
