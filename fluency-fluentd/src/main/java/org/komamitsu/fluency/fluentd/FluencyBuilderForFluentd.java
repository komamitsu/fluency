...
public class FluencyBuilderForFluentd extends org.komamitsu.fluency.FluencyBuilder {
  ...
  public Fluency build(String host, int port) {
    return buildFromIngester(recordFormatter, buildIngester(createBaseSender(host, port)));
  }

  public Fluency build(int port) {
    return buildFromIngester(recordFormatter, buildIngester(createBaseSender(null, port)));
  }

  public Fluency build() {
    return buildFromIngester(recordFormatter, buildIngester(createBaseSender(null, null)));
  }
  ...
  public Fluency build(List<InetSocketAddress> servers) {
    ...
    return buildFromIngester(recordFormatter, buildIngester(new MultiSender(senders)));
  }
  ...
  protected Ingester buildIngester(FluentdSender baseSender) {
    ...
    RetryableSender retryableSender =
        new RetryableSender(
            senderConfig, baseSender, new ExponentialBackOffRetryStrategy(retryStrategyConfig));
