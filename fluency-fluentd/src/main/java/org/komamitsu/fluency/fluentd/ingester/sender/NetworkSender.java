...
public abstract class NetworkSender<T> extends FluentdSender {
  ...
  abstract void sendBuffers(T socket, List<ByteBuffer> buffers) throws IOException;

  abstract void recvResponse(T socket, ByteBuffer buffer) throws IOException;
  ...
  protected synchronized void sendInternal(List<ByteBuffer> buffers, String ackToken)
    ...
    try {
      ...
      if (!ackToken.equals(response.getAck())) {
        throw new UnmatchedAckException(
            "Ack tokens don't matched: expected=" + ", got=" + response.getAck());
      }
    } catch (IOException e) {
