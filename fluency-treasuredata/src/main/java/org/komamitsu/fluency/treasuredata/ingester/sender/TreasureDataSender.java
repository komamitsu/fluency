...
public class TreasureDataSender implements Closeable, Sender {
  ...
  private void createDatabase(String database) {
    ...
    throw new NonRetryableException(
        String.format(
            "It seems you don't have a proper permission on the database. database=%s", database));
  }
  ...
  private void importData(String database, String table, String uniqueId, File file) {
    LOG.debug(
        "Importing data to TD table: database={}, table={}, uniqueId={}, fileSize={}",
        database,
        table,
        uniqueId,
        file.length());
      ...
      try (InputStream in = new ByteBufferBackedInputStream(dataBuffer);
          OutputStream out =
              new GZIPOutputStream(
                  Files.newOutputStream(file.toPath(), StandardOpenOption.WRITE))) {
